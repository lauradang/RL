...
class TestAsyncTrajectoryCollector:
    ...
    def test_backend_wait_ignores_enqueue_pending_rollouts(self):
        """Pre-refit backend wait should not block on replay-buffer enqueue."""
        buffer = ReplayBuffer.remote(max_size=10)
        mock_generation = MockGenerationInterface()
        mock_tokenizer = mock.MagicMock()
        mock_env = MockEnvironment.remote(rewards=[1.0, 2.0])
        ...
        ray.get(collector._begin_backend_request.remote())
        ray.get(collector._finish_backend_request.remote(pending_enqueue=True))

        stats = ray.get(collector.get_backend_request_stats.remote())
        assert stats["backend_active_threads"] == 0
        assert stats["backend_enqueue_wait_threads"] == 1

        start_time = time.time()
        wait_metrics = ray.get(collector.wait_for_backend_requests.remote())
        assert time.time() - start_time < 1.0
        assert wait_metrics["pre_refit_backend_active_threads_before_wait"] == 0
        assert (
            wait_metrics["pre_refit_backend_enqueue_wait_threads_before_wait"] == 1
        )
        assert wait_metrics["pre_refit_waited_for_backend_threads"] == 1

        ray.get(collector._finish_backend_enqueue.remote())
        stats = ray.get(collector.get_backend_request_stats.remote())
        assert stats["backend_enqueue_wait_threads"] == 0

        ray.kill(collector)
        ray.kill(buffer)
        ray.kill(mock_env)

    def test_nemo_gym_prepare_for_refit_waits_for_backend_phase_only(self):
        """NeMo-Gym refit quiesces backend-active rollout work, not all threads."""
        buffer = ReplayBuffer.remote(max_size=10)
        mock_generation = MockGenerationInterface()
        mock_tokenizer = mock.MagicMock()
        mock_env = MockEnvironment.remote(rewards=[1.0, 2.0])
        task_to_env = {"nemo_gym": mock_env}
        master_config = self.create_mock_config()
        ...
        metrics = ray.get(collector.prepare_for_refit.remote())
        assert metrics["pre_refit_waited_for_backend_threads"] == 1
        assert metrics["pre_refit_waited_for_inflight_threads"] == 0
        assert "pre_refit_prepare_for_refit_time" in metrics
