...
#### Weighted Dataset Mixing with SingleController

SingleController can build each optimizer step from deterministic per-dataset
quotas. Set `data.use_multiple_dataloader: true` and give every training dataset
an explicit relative `weight`:
...
This produces exactly 24 prompts from the first task and 8 from the second on
every step. Weights are normalized and converted to integer quotas with stable
largest-remainder apportionment. Each task keeps its own environment routing.
Replay-buffer selection also enforces the remaining per-task quota, so a faster
environment cannot displace a slower one when rollouts complete out of order.

Rules:

- Every non-evaluation dataset must declare a non-negative weight, and their
  total weight must be positive.
- Every positive-weight task must receive at least one prompt per step.
- Weighted mixing is incompatible with dynamic sampling.
- `evaluation_only` datasets are rejected by SingleController until its
  validation path is implemented.
- The weighted loader is finite and checkpointable; an epoch ends when the
  shortest per-task dataloader is exhausted.

Use `examples/configs/grpo_weighted_multienv_single_controller.yaml` as a
runnable two-environment smoke configuration with
